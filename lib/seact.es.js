...
import require$$0$1, { Component } from "react";
    ...
    componentDidMount() {
      ...
      const SanContainer = san.defineComponent({
        ...
        attached() {
          ...
          const eventObj = getEvent$1(self2);
          Object.keys(eventObj).forEach((key) => {
            self2.sanApp.on(key, eventObj[key]);
          });
          if (self2.props.sModels) {
            lodash.exports.forOwn(self2.props.sModels, (value, key) => {
...
function getData(self2) {
  ...
  return {
    ...lodash.exports.omit(self2.props, ["sModels", "children"]),
    ...obj
  };
}
...
function isNormalObject(children) {
  return !Array.isArray(children) && !require$$0$1.isValidElement(children) && children instanceof Object;
}
function Container(ReactComponent) {
  ...
  return _a = class extends san.Component {
    ...
    attached() {
      ...
      this.reactDOMRoot = client.createRoot(this.el);
      this.render();
    }
    updated() {
      this.render();
...
function getBind(self2) {
  ...
  for (const item of self2.binds.filter((v) => v.name !== "slot")) {
    bindObj[item.name] = self2.parent.data.get(item.expr);
    bindObj[`${item.name}Change`] = (...porps) => {
      self2.parent.data.set(item.expr, ...porps);
    };
  }
  return bindObj;
}
